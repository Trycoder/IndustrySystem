...
Partial Class UnitMaster
    ...
    Protected Sub btnSave_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSave.Click
        If UCase(btnSave.Text) = UCase("Update") Then
            If Not String.IsNullOrEmpty(rowid) Then
                ...
                Dim str As String
                con.Open()
                str = "update tbl_UnitMaster set unitcode ='" & Trim(txtcategorycode.Text.Replace("'", "''")) & "',unitdesc ='" & Trim(txtcategorydesc.Text.Replace("'", "''")) & "' where id=" & rowid & ""
        ...
        Else
            ...
            con.Open()
            cmd.Connection = con
            Sql = "Select * from tbl_UnitMaster where unitdesc like '" & Trim(txtcategorydesc.Text.Replace("'", "''")) & "'  "
            cmd.CommandText = Sql
            rdr = cmd.ExecuteReader
            If rdr.HasRows Then
                Dim myscript As String = "alert('Unit already exists.. ');"
            ...
            Dim str As String
            con.Open()
            str = "insert into tbl_UnitMaster(unitcode,unitdesc) values ('" & txtcategorycode.Text.Replace("'", "''") & "','" & txtcategorydesc.Text.Replace("'", "''") & "')"
            cmd.CommandText = str
            cmd.Connection = con
            cmd.ExecuteScalar()
            con.Close()
    ...
    Private Function bindCategoryDetails(ByVal id As String)
        Try
            con.Open()
            cmd = New SqlCommand("select * from tbl_UnitMaster where id=" & id & " order by unitcode", con)
            rdr = cmd.ExecuteReader
            If rdr.HasRows Then
                If rdr.Read Then
                    txtcategorycode.Text = rdr("unitcode")
                    txtcategorydesc.Text = rdr("unitdesc")
                End If
            End If
            con.Close()
            btnSave.Text = "Update"
    ...
    Public Function BindGrid(ByVal sortExpression As String, ByVal direction As String, Optional ByVal condition As String = "")
        Try
            ...
            Else
                sql = "select * from tbl_UnitMaster where " & condition & " order by unitcode"
            End If
    ...
    Protected Sub grdreportcategory_PageIndexChanging(sender As Object, e As GridViewPageEventArgs) Handles grdreportcategory.PageIndexChanging
        ...
        If drpreportsearch.SelectedValue = "UnitCode" And txtshsearch.Text <> "" Then
            condition = drpreportsearch.SelectedValue & " like '%" & txtshsearch.Text & "%'"
            BindGrid("", "", condition)
        ElseIf drpreportsearch.SelectedValue = "UnitDesc" Then
            condition = drpreportsearch.SelectedValue & " like '%" & txtshsearch.Text & "%'"
            BindGrid("", "", condition)
        Else
            BindGrid("", "")
    ...
    Protected Sub grdreportcategory_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles grdreportcategory.RowCommand
        Try
            If e.CommandName = "Deleterow" Then
                ...
                End If
                con.Open()
                cmd = New SqlCommand("Select count(unitid) from tbl_Material where unitid =" & repcatid, con)
                Dim str As String = cmd.ExecuteScalar
                con.Close()
                cmd.Dispose()
                If str <> "0" Then
                    Dim myscript As String = "alert('Unit Mapped with Material');"
                    Page.ClientScript.RegisterStartupScript(Me.GetType(), "myscript", myscript, True)
                Else
                    con.Open()
                    cmd = New SqlCommand("delete from tbl_UnitMaster where id=" & repcatid, con)
                    cmd.ExecuteNonQuery()
                    con.Close()
                    BindGrid("", "")
                    Dim myscript As String = "alert('Unit Deleted Successfully!');"
                    Page.ClientScript.RegisterStartupScript(Me.GetType(), "myscript", myscript, True)
                End If
            End If
        Catch ex As Exception
    ...
    Protected Sub GridView1_RowDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles grdreportcategory.RowDataBound
        If e.Row.RowType = DataControlRowType.DataRow Then
            ...
            If lnkedit IsNot Nothing Then
                lnkedit.NavigateUrl = "UnitMaster.aspx?Id=" & grdreportcategory.DataKeys(e.Row.RowIndex).Value
            End If
        End If
        If e.Row.RowType = DataControlRowType.DataRow Then
            If Session("Usergroup") <> "1" Then
