...
Partial Class EmployeeCategory
    ...
    Protected Sub btnSave_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSave.Click
        If UCase(btnSave.Text) = UCase("Update") Then
            If Not String.IsNullOrEmpty(rowid) Then
                ...
                Dim str As String
                con.Open()
                str = "update tbl_EmployeeCategory set empcatcode ='" & Trim(txtcategorycode.Text.Replace("'", "''")) & "',empcatname  ='" & Trim(txtcategorydesc.Text.Replace("'", "''")) & "' where empcatid=" & rowid & ""
        ...
        Else
            ...
            con.Open()
            cmd.Connection = con
            Sql = "Select * from tbl_EmployeeCategory where empcatcode like '" & Trim(txtcategorycode.Text.Replace("'", "''")) & "'  "
            cmd.CommandText = Sql
            rdr = cmd.ExecuteReader
            If rdr.HasRows Then
                Dim myscript As String = "alert('Category code already exists.. ');"
            ...
            Dim str As String
            con.Open()
            str = "insert into tbl_EmployeeCategory(empcatcode,empcatname,whrs,wdays,attbonus,sundaybonus,lunchded) values ('" & txtcategorycode.Text.Replace("'", "''") & "','" & txtcategorydesc.Text.Replace("'", "''") & "',"
            str = str & Val(TxtWHrs.Text) & "," & Val(TxtWDays.Text) & "," & Val(TxtAB.Text) & "," & Val(TxtSB.Text) & "," & Val(TxtLD.Text) & ")"
            cmd.CommandText = str
            cmd.Connection = con
            cmd.ExecuteScalar()
            con.Close()
    ...
    Private Function bindCategoryDetails(ByVal id As String)
        Try
            con.Open()
            cmd = New SqlCommand("select * from tbl_EmployeeCategory where empcatid =" & id & " order by empcatcode", con)
            rdr = cmd.ExecuteReader
            If rdr.HasRows Then
                If rdr.Read Then
                    txtcategorycode.Text = rdr("empcatcode")
                    txtcategorydesc.Text = rdr("empcatname")
                    TxtWHrs.Text = rdr("whrs")
                    TxtWDays.Text = rdr("wdays")
                    TxtAB.Text = rdr("attbonus")
                    TxtSB.Text = rdr("sundaybonus")
                    TxtLD.Text = rdr("lunchded")
                End If
            End If
            con.Close()
            btnSave.Text = "Update"
    ...
    Public Function BindGrid(ByVal sortExpression As String, ByVal direction As String, Optional ByVal condition As String = "")
        Try
            ...
            Else
                sql = "select * from tbl_EmployeeCategory where " & condition & " order by empcatcode"
            End If
    ...
    Protected Sub grdreportcategory_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles grdreportcategory.RowCommand
        Try
            If e.CommandName = "Deleterow" Then
                ...
                End If
                con.Open()
                cmd = New SqlCommand("delete from tbl_EmployeeCategory where empcatid =" & repcatid, con)
                cmd.ExecuteNonQuery()
                con.Close()
                BindGrid("", "")
                Dim myscript As String = "alert('Category Code Deleted Successfully!');"
                Page.ClientScript.RegisterStartupScript(Me.GetType(), "myscript", myscript, True)
            End If
        Catch ex As Exception
            Dim errorscript As String = "alert('Error - " & ex.Message.ToString().Replace("'", "") & "');"
    ...
    Protected Sub GridView1_RowDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles grdreportcategory.RowDataBound
        If e.Row.RowType = DataControlRowType.DataRow Then
            ...
            If lnkedit IsNot Nothing Then
                lnkedit.NavigateUrl = "EmployeeCategory.aspx?Id=" & grdreportcategory.DataKeys(e.Row.RowIndex).Value
            End If
        End If
        If e.Row.RowType = DataControlRowType.DataRow Then
            If Session("Usergroup") <> "1" Then
