...
Partial Class LeaveType
    ...
    Protected Sub btnSave_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSave.Click
        If UCase(btnSave.Text) = UCase("Update") Then
            If Not String.IsNullOrEmpty(rowid) Then
                ...
                Dim str As String
                con.Open()
                str = "update tbl_leavetype set leavetypecode ='" & Trim(txtcategorycode.Text.Replace("'", "''")) & "',leavetypename  ='" & Trim(txtcategorydesc.Text.Replace("'", "''")) & "' where leavetypeid=" & rowid & ""
        ...
        Else
            ...
            con.Open()
            cmd.Connection = con
            Sql = "Select * from tbl_leavetype where leavetypecode like '" & Trim(txtcategorycode.Text.Replace("'", "''")) & "'  "
            cmd.CommandText = Sql
            rdr = cmd.ExecuteReader
            If rdr.HasRows Then
                Dim myscript As String = "alert('leave type code already exists.. ');"
            ...
            Dim str As String
            con.Open()
            str = "insert into tbl_leavetype(leavetypecode,leavetypename) values ('" & txtcategorycode.Text.Replace("'", "''") & "','" & txtcategorydesc.Text.Replace("'", "''") & "')"
            cmd.CommandText = str
            cmd.Connection = con
            cmd.ExecuteScalar()
            con.Close()
    ...
    Private Function bindCategoryDetails(ByVal id As String)
        Try
            con.Open()
            cmd = New SqlCommand("select * from tbl_leavetype where leavetypeid =" & id & " order by leavetypecode", con)
            rdr = cmd.ExecuteReader
            If rdr.HasRows Then
                If rdr.Read Then
                    txtcategorycode.Text = rdr("leavetypecode")
                    txtcategorydesc.Text = rdr("leavetypename")
                End If
            End If
            con.Close()
            btnSave.Text = "Update"
    ...
    Public Function BindGrid(ByVal sortExpression As String, ByVal direction As String, Optional ByVal condition As String = "")
        Try
            ...
            If condition = "" Then
                sql = "select * from tbl_leavetype order by leavetypecode"
            Else
                sql = "select * from tbl_leavetype where " & condition & " order by leavetypecode"
            End If
    ...
    Protected Sub grdreportcategory_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles grdreportcategory.RowCommand
        Try
            If e.CommandName = "Deleterow" Then
                ...
                End If
                con.Open()
                cmd = New SqlCommand("delete from tbl_leavetype where leavetypeid=" & repcatid, con)
                cmd.ExecuteNonQuery()
                con.Close()
                BindGrid("", "")
                Dim myscript As String = "alert('leave type Deleted Successfully!');"
                Page.ClientScript.RegisterStartupScript(Me.GetType(), "myscript", myscript, True)
            End If
        Catch ex As Exception
            Dim errorscript As String = "alert('Error - " & ex.Message.ToString().Replace("'", "") & "');"
    ...
    Protected Sub GridView1_RowDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles grdreportcategory.RowDataBound
        If e.Row.RowType = DataControlRowType.DataRow Then
            ...
            If lnkedit IsNot Nothing Then
                lnkedit.NavigateUrl = "leavetype.aspx?Id=" & grdreportcategory.DataKeys(e.Row.RowIndex).Value
            End If
        End If
        If e.Row.RowType = DataControlRowType.DataRow Then
            If Session("Usergroup") <> "1" Then
